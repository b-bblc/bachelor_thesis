...
Following PCA dimensionality reduction, we applied cluster optimisation methods. Figure~\ref{fig:cluster_opt_app} presents the results of these analyses. The left panel shows K-means inertia across cluster counts $k = 2$ to $11$, while the right panel displays the corresponding average silhouette scores. The inertia curve exhibits a gradual leveling around $k = 4$ to $5$, while the silhouette analysis achieves its maximum at $k = 4$ with an average score of approximately 0.35. This convergence of evidence from multiple optimisation methods supports the selection of $k = 4$ as the optimal clustering solution.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{images/determining_optimal_number_of_clusters_2_7.png}
  \caption{Cluster optimisation results. \textit{Left:} Elbow plot showing K-means inertia decrease. \textit{Right:} Mean silhouette scores across different $k$ values. The optimal solution ($k=4$) is indicated at the elbow point with the highest silhouette score.}
